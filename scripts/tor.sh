...
do
    ...
    PREFIX="${CONFIGURATION_TEMP_DIR}/tor-${ARCH}"
    ./configure --disable-tool-name-check --disable-unittests --enable-static-openssl --enable-static-libevent --disable-asciidoc --disable-transparent --disable-system-torrc --disable-linker-hardening --disable-dependency-tracking --prefix="${PREFIX}" --with-libevent-dir="${BUILT_PRODUCTS_DIR}" --with-openssl-dir="${BUILT_PRODUCTS_DIR}" CC="$(xcrun -f --sdk ${PLATFORM_NAME} clang) -arch ${ARCH}" CPP="$(xcrun -f --sdk ${PLATFORM_NAME} clang) -E -arch ${ARCH}  -miphoneos-version-min=${IOS_MIN_SDK_VERSION}" CPPFLAGS="${BITCODE_FLAGS} -I${BUILT_PRODUCTS_DIR}/libevent-${ARCH} -miphoneos-version-min=${IOS_MIN_SDK_VERSION} -I${BUILT_PRODUCTS_DIR}/openssl -I${PSEUDO_SYS_INCLUDE_DIR}" cross_compiling="yes" ac_cv_func__NSGetEnviron="no" ac_cv_func_clock_gettime="no" ac_cv_func_getentropy="no" LDFLAGS="-lz ${BITCODE_FLAGS}"
...
TOR_REVISION=$(find "${BUILT_PRODUCTS_DIR}/tor-${ARCHS[0]}/" -name "micro-revision.i" -exec cat {} \;)
echo const char tor_git_revision[] = "${TOR_REVISION}"\; > "../../Source/tor.c"